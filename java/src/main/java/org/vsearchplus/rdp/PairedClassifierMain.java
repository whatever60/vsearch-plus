...
/**
 * CLI entrypoint for native paired-end TAV taxonomy assignment using stock RDP
 * internals.
 */
public final class PairedClassifierMain {
  ...
  public static void main(final String[] args) throws Exception {
    ...
    if (!parsed.interleaved && parsed.inputFile2 == null) {
      throw new IllegalArgumentException(
          "--input2 is required unless --interleaved is set");
    }
    if (parsed.interleaved && parsed.inputFile2 != null) {
      throw new IllegalArgumentException(
          "--input2 is not allowed with --interleaved");
    }
    ...
    final String gene =
        parsed.gene == null ? ClassifierFactory.RRNA_16S_GENE : parsed.gene;
    ...
    final MCSample aggregateSample =
        needsAggregateOutputs ? new MCSample("paired_tav") : null;
    final List<MCSample> aggregateSamples = new ArrayList<MCSample>();
    if (aggregateSample != null) {
      aggregateSamples.add(aggregateSample);
    }
    final ConcretRoot<MCTaxon> hierRoot =
        (parsed.hierOutFile != null)
            ? new ConcretRoot<MCTaxon>(new MCTaxon(factory.getRoot().getTaxid(),
                                                   factory.getRoot().getName(),
                                                   factory.getRoot().getRank()))
            : null;
    final HashMap<String, Long> seqCountMap = new HashMap<String, Long>();
    final HashMap<Taxon, Double> cachedCopyNumber =
        new HashMap<Taxon, Double>();

    try (PrintWriter outWriter = new PrintWriter(parsed.outputFile);
         PrintWriter shortSeqWriter =
             parsed.shortSeqFile == null ? null
                                         : new PrintWriter(parsed.shortSeqFile);
         PairReader pairReader = new PairReader(
             parsed.inputFile, parsed.inputFile2, parsed.interleaved)) {
      if (ClassificationResultFormatter.FORMAT.filterbyconf.equals(
              parsed.outputFormat)) {
      ...
      while ((pair = pairReader.nextPair()) != null) {
        try {
          final ClassifierSequence leftSeq =
              new ClassifierSequence(pair.leftSeq);
          final ClassifierSequence rightSeq =
              new ClassifierSequence(pair.rightSeq);
          final ClassificationResult result = pairedClassifier.classifyPair(
              pair.outputId, leftSeq, rightSeq, parsed.minWords);
          outWriter.print(formatResult(result, parsed.outputFormat,
                                       parsed.confidence, ranks));
          if (aggregateSample != null) {
            aggregateSample.addRankCount(result);
          }
          if (hierRoot != null) {
            processClassificationResult(result, aggregateSample, hierRoot,
                                        parsed.confidence, seqCountMap,
                                        hasCopyNumber, cachedCopyNumber);
          }
    ...
    if (parsed.hierOutFile != null) {
      ...
      try (PrintStream hierOut = new PrintStream(hierOutFile)) {
        final DefaultPrintVisitor printVisitor =
            new DefaultPrintVisitor(hierOut, aggregateSamples);
        hierRoot.topDownVisit(printVisitor);
      }

      if (hasCopyNumber) {
        final File parent = hierOutFile.getParentFile();
        final File copyNumberAdjusted =
            (parent == null)
        ...
        try (PrintStream adjustedOut = new PrintStream(copyNumberAdjusted)) {
          final DefaultPrintVisitor adjustedVisitor =
              new DefaultPrintVisitor(adjustedOut, aggregateSamples, true);
    ...
    if (parsed.bootstrapOutFile != null) {
      try (PrintStream bootstrapOut =
               new PrintStream(parsed.bootstrapOutFile)) {
        MCSamplePrintUtil.printBootstrapCountTable(bootstrapOut,
                                                   aggregateSample);
      }
    }
  }
  ...
  formatResult(final ClassificationResult result,
               final ClassificationResultFormatter.FORMAT outputFormat,
               final float confidence, final String[] ranks) {
    return ClassificationResultFormatter.getOutput(result, outputFormat,
                                                   confidence, ranks);
  }
  ...
  /**
   * Reflection helper to reuse stock-loaded training objects without
   * retraining.
   */
  private static TrainingInfo
  extractTrainingInfo(final ClassifierFactory factory) throws Exception {
    final Field trainingInfoField =
        ClassifierFactory.class.getDeclaredField("trainingInfo");
  ...
  private static Args parseArgs(final String[] args) {
    ...
    if (ClassificationResultFormatter.FORMAT.biom.equals(parsed.outputFormat)) {
      throw new IllegalArgumentException(
          "--format biom is not supported for paired TAV mode");
    }
    if (valueOptions.containsKey("--metadata")) {
      throw new IllegalArgumentException(
          "--metadata is not supported for paired TAV mode");
    }
    if (valueOptions.containsKey("--biomFile")) {
      throw new IllegalArgumentException(
          "--biomFile is not supported for paired TAV mode");
    }
  ...
  private static String normalizeOption(final String token) {
    ...
    if ("--output".equals(token) || "--outputFile".equals(token) ||
        "-o".equals(token)) {
      return "--output";
    }
    if ("--shortseq-outfile".equals(token) ||
        "--shortseq_outfile".equals(token) || "-s".equals(token)) {
      return "--shortseq-outfile";
    }
    if ("--train-prop".equals(token) || "--train_propfile".equals(token) ||
        "-t".equals(token)) {
      return "--train-prop";
    }
    ...
    if ("--min-words".equals(token) || "--minWords".equals(token) ||
        "-w".equals(token)) {
      return "--min-words";
    }
  ...
  /**
   * Stock-style helper: find or create one taxon node under the aggregate
   * output tree.
   */
  private static MCTaxon findOrCreateTaxon(final ConcretRoot<MCTaxon> root,
    ...
    if (taxon == null) {
      taxon = new MCTaxon(assignment.getTaxid(), assignment.getName(),
                          assignment.getRank(), unclassified);
  ...
  /**
   * Stock-style helper: resolve cached copy number for lowest confident
   * assignment taxon.
   */
  private static double findCopyNumber(final RankAssignment assignment,
                                       final Taxon taxon,
  ...
  /**
   * Stock-style helper: update aggregate hierarchy tree and counts for one
   * classification result.
   */
  private static void processClassificationResult(
      final ClassificationResult result, final MCSample sample,
      final ConcretRoot<MCTaxon> root, final float confidence,
      final Map<String, Long> seqCountMap, final boolean hasCopyNumber,
    ...
    for (RankAssignment assignment : result.getAssignments()) {
      ...
      if (assignment.getConfidence() < confidence) {
        ...
        countTaxon = taxon;
        taxon = findOrCreateTaxon(root, lastAssignment, parentId, true,
                                  seqCountMap, lineage.toString());
        stop = true;
      } else {
        if (lastAssignment != null) {
          parentId = lastAssignment.getTaxid();
        }
        taxon = findOrCreateTaxon(root, assignment, parentId, false,
                                  seqCountMap, lineage.toString());
        countTaxon = taxon;
      }
      ...
      lineage.append(assignment.getName())
          .append(";")
          .append(assignment.getRank())
          .append(";");
    }

    if (hasCopyNumber && (countTaxon != null) &&
        (!result.getAssignments().isEmpty())) {
      final RankAssignment last =
          result.getAssignments().get(result.getAssignments().size() - 1);
      final double copyNumber =
          findCopyNumber(last, countTaxon, cachedCopyNumber);
   ...
   */
  private static ClassificationResultFormatter.FORMAT
  parseOutputFormat(final String formatText) {
  ...
  private static void printUsage() {
    ...
    System.out.println(
        "  -g,--gene <name>             16srrna|fungallsu|fungalits_warcup|fungalits_unite");
  ...
  private static final class PairRecord {
    ...
    private PairRecord(final Sequence leftSeq, final Sequence rightSeq,
  ...
  private static final class PairReader implements AutoCloseable {
    ...
    private PairReader(final String inputFile, final String inputFile2,
    ...
    private PairRecord nextPair() throws IOException {
      ...
      if (interleaved) {
        ...
        if (right == null) {
          throw new IllegalArgumentException(
              "Interleaved input has odd number of records");
        }
      } else {
        left = readerLeft.readNextSequence();
        ...
        if (left == null || right == null) {
          throw new IllegalArgumentException(
              "Input files have different number of records");
        }
      }
      ...
      if (!leftComparableId.equals(rightComparableId)) {
        throw new IllegalArgumentException(
            "Paired sequence ID mismatch: " + left.getSeqName() + " vs " +
            right.getSeqName());
      }
