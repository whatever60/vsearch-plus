...
auto make_tav_aligner_workers(struct Scoring const & scoring,
                              ...
                              int64_t const max_workers) -> std::vector<TavAlignerWorker>
{
  auto const bounded_threads = std::max<int64_t>(1, std::min<int64_t>(requested_threads, max_workers));
  ...
  for (auto & worker : workers)
    {
      ...
      worker.simd_left = search16_init(opt_match,
                                       opt_mismatch,
      ...
      worker.simd_right = search16_init(opt_match,
                                        opt_mismatch,
                                        opt_gap_open_query_left,
                                        opt_gap_open_target_left,
                                        opt_gap_open_query_interior,
...
auto destroy_tav_aligner_workers(std::vector<TavAlignerWorker> & workers) -> void
{
  for (auto & worker : workers)
    {
...
auto tav_parallel_for(std::size_t const task_count,
                      std::vector<TavAlignerWorker> & workers,
                      Func && fn) -> void
{
  ...
  auto run_worker = [&](std::size_t const worker_index) -> void {
    for (std::size_t task_index = worker_index; task_index < task_count; task_index += worker_count)
      {
                            ...
                            int const seqmask) -> TavPairedQuerySearch
{
  ...
  unique_count(unique_handle,
               wordlength,
               static_cast<int>(query_left.size()),
               query_left.c_str(),
               &result.qk_left,
               &qk_left_list,
               seqmask);
  ...
  unique_count(unique_handle,
               wordlength,
               static_cast<int>(query_right.size()),
               query_right.c_str(),
               &result.qk_right,
               &qk_right_list,
               seqmask);
  for (auto i = 0U; i < result.qk_right; ++i)
    {
      auto const right_it = target_kmer_index.right_postings.find(qk_right_list[i]);
...
auto search_joinhits_paired(std::vector<unsigned int> const & target_kmer_scores,
                            ...
                            int64_t const tophits) -> std::vector<std::pair<int, unsigned int>>
{
  ...
  for (std::size_t i = 0; i < targets.size(); ++i)
    {
      ...
      novel.length = static_cast<unsigned int>(targets[i].left.size() + targets[i].right.size());
...
auto build_tav_align_stats(char * cigar,
                           int64_t const nwscore,
                           int64_t const nwalignmentlength,
                           int64_t const nwmatches,
                           int64_t const nwmismatches,
                           int64_t const nwgaps,
                           int64_t const query_len,
                           ...
                           bool const keep_alignment_string) -> TavAlignStats
{
  ...
  h.shortest = std::min(static_cast<int>(query_len), static_cast<int>(target_len));
  h.longest = std::max(static_cast<int>(query_len), static_cast<int>(target_len));
...
auto align_end_batch_stock_style(std::string const & query,
                                 ...
                                 bool const keep_alignment_string) -> std::vector<TavAlignStats>
{
  ...
  search16_qprep(simd_aligner, query_buf.data(), static_cast<int>(query.size()));
  ...
  search16(simd_aligner,
           static_cast<unsigned int>(targets.size()),
           targets.data(),
           simd_nwscore.data(),
           simd_nwalignmentlength.data(),
           simd_nwmatches.data(),
           simd_nwmismatches.data(),
           simd_nwgaps.data(),
           cigars.data());
  ...
  for (std::size_t i = 0; i < target_seqnos.size(); ++i)
    {
      ...
      if (nwscore == std::numeric_limits<short>::max())
        {
          ...
          auto const target_len = static_cast<int64_t>(db_getsequencelen(target_seqnos[i]));
          cigar = xstrdup(lma.align(query_buf.data(),
                                    target_seq,
      ...
      stats[i] = build_tav_align_stats(cigar,
                                       nwscore,
                                       nwalignmentlength,
                                       nwmatches,
                                       nwmismatches,
                               ...
                               LinearMemoryAligner & lma) -> TavAlignStats
{
  ...
  auto stats = build_tav_align_stats(cigar,
                                     nwscore,
                                     nwalignmentlength,
                                     nwmatches,
                                     nwmismatches,
                                   ...
                                   TavRecord const & target) -> bool
{
  ...
  numeric_filters.qlen = static_cast<double>(query.left.size() + query.right.size());
  numeric_filters.tlen = static_cast<double>(target.left.size() + target.right.size());
                                 ...
                                 TavHit & hit) -> bool
{
  ...
  filter_input.trim_right_total =
    ...
    hit.right.trim_q_right + hit.right.trim_t_right;
                                    ...
                                    bool const apply_qmask) -> TavLoadResult
{
  ...
  while (fastx_next(left_h, not opt_notrunclabels, chrmap_no_change_vector.data()))
    {
      ...
      auto const left_seq = std::string{fastx_get_sequence(left_h), static_cast<std::size_t>(left_len)};

      std::string right_seq;
      int64_t right_len = 0;
      if (interleaved)
        {
          if (!fastx_next(left_h, not opt_notrunclabels, chrmap_no_change_vector.data()))
            {
              fatal("Odd number of records in interleaved paired FASTX input %s; expected left/right entries", left_filename);
            }
          right_len = static_cast<int64_t>(fastx_get_sequence_length(left_h));
          right_seq.assign(fastx_get_sequence(left_h), static_cast<std::size_t>(right_len));
        }
      else
        {
          ...
          right_seq.assign(fastx_get_sequence(right_h), static_cast<std::size_t>(right_len));
        }
      ...
      if (apply_qmask and
          ((opt_qmask == MASK_DUST) or ((opt_qmask == MASK_SOFT) and (opt_hardmask != 0))))
        {
  ...
  if ((not interleaved) and
      fastx_next(right_h, not opt_notrunclabels, chrmap_no_change_vector.data()))
    {
...
auto write_paired_fasta_split(std::FILE * fp_left,
                              std::FILE * fp_right,
...
auto tav_fastx_uniques(struct Parameters const & parameters) -> void
{
  if (parameters.opt_interleaved and (parameters.opt_reverse != nullptr))
    {
      fatal("TAV fastx_uniques paired input must be either split (R1 + R2 positional input) or --interleaved, not both");
    }
  if ((not parameters.opt_interleaved) and (parameters.opt_reverse == nullptr))
    {
      fatal("TAV fastx_uniques requires paired input: provide R2 as positional input or set --interleaved");
    }
  ...
  auto load_result = load_paired_records_from_fastx(parameters.opt_fastx_uniques,
...
auto tav_cluster_unoise(struct Parameters const & parameters) -> void
{
  ...
  if ((not parameters.opt_interleaved) and (parameters.opt_reverse == nullptr))
    {
      fatal("Paired cluster_unoise requires paired input: provide R2 as positional input or set --interleaved (catalog mode is not supported)");
    }
  ...
  auto tav_aligner_workers = make_tav_aligner_workers(scoring,
  ...
  for (auto const & q : records)
    {
      ...
      if (!centroids.empty())
        {
          auto const query_search = search_onequery_paired(q.left,
                                                           q.right,
                                                           kmer_index,
                                                           centroids.size(),
                                                           kmer_unique_handle,
          ...
          auto process_delayed = [&]() -> void {
            ...
            for (std::size_t delayed_index = 0; delayed_index < delayed.size(); ++delayed_index)
              {
                ...
                auto const left_seqno = static_cast<unsigned int>(2 * hit.centroid_index);
            ...
            if (!alignable_delayed_indices.empty())
              {
                ...
                auto left_stats = align_end_batch_stock_style(q.left,
                                                              left_target_seqnos,
                                                              worker.simd_left,
                                                              *worker.lma,
                                                              false);
                auto right_stats = align_end_batch_stock_style(q.right,
                                                               right_target_seqnos,
                                                               worker.simd_right,
                                                               *worker.lma,
                                                               false);
                ...
                for (std::size_t i = 0; i < alignable_delayed_indices.size(); ++i)
                  {
                    ...
                    aligned_pass[delayed_index] =
                      paired_aligned_filters_pass(q, centroid, hit)
                      ? 1U
                      : 0U;
                  }
              }
  ...
  if (!opt_quiet)
    {
      std::fprintf(stderr,
                   "TAV cluster_unoise: %zu input pairs (%" PRId64 " filtered by minsize, %" PRId64 " filtered by maxsize) -> %zu centroids; candidates=%" PRId64 ", aligned=%" PRId64 ", accepted=%" PRId64 ", weak=%" PRId64 ", rejected=%" PRId64 "\n",
                   loaded_pairs,
                   filtered_small,
                   filtered_large,
                   centroids.size(),
...
auto tav_uchime3_denovo(struct Parameters const & parameters) -> void
{
  ...
  if ((not parameters.opt_interleaved) and (parameters.opt_reverse == nullptr))
    {
      fatal("Paired uchime3_denovo requires paired input: provide R2 as positional input or set --interleaved (catalog mode is not supported)");
    }
  ...
  auto tav_aligner_workers = make_tav_aligner_workers(scoring,
                                                      parameters.opt_threads,
                                                      static_cast<int64_t>(chimera_tophits));
  reset_simd_target_db();
  for (auto const & r : records)
    {
  ...
  auto select_best_parent_pair = [&](std::vector<TavParentCandidate> const & candidates,
    ...
    std::vector<int> match(static_cast<std::size_t>(cand_count) * static_cast<std::size_t>(query_len), 0);
    for (auto c = 0; c < cand_count; ++c)
      {
        ...
        for (std::size_t qpos = 0; qpos < copy_len; ++qpos)
          {
            match[(static_cast<std::size_t>(c) * static_cast<std::size_t>(query_len)) + qpos] = axis[qpos] != 0U;
          }
      }

    std::array<int, 2> best_parent_idx {{-1, -1}};
    auto const found = select_best_two_parents_from_match_matrix(match,
                                                                  cand_count,
                                                                  query_len,
                                                                  chimera_window,
                                                                  best_parent_idx);
  ...
  auto evaluate_parent_pair = [&](TavRecord const & q,
    ...
    auto build_end_alignments = [](std::string const & query,
      ...
      auto const cigar_a_operations = parse_cigar_operations_from_string(cigar_a);
      auto const cigar_b_operations = parse_cigar_operations_from_string(cigar_b);

      std::vector<int> maxi(query.size() + 1U, 0);

      auto fill_maxi = [&](std::vector<std::pair<Operation, long long>> const & cigar_operations) -> void {
        auto qpos = std::size_t{0};
        for (auto const & operation_and_run : cigar_operations)
          {
            auto const operation = operation_and_run.first;
            ...
            else if ((operation == Operation::match) or (operation == Operation::deletion))
              {
                qpos += run;
              }
          }
      ...
      auto build_parent_alignment = [&](std::string const & target,
                                        std::vector<std::pair<Operation, long long>> const & cigar_operations,
                                        std::string & out) -> void {
        out.clear();
            ...
            if (operation == Operation::insertion)
              {
                ...
                for (int i = 0; i < ins; ++i)
                  {
                    if ((static_cast<std::size_t>(i) < run) and (tpos < target.size()))
                      {
    ...
    for (auto scan_i = 0; scan_i < static_cast<int>(scan_index.size()); ++scan_i)
      {
        alignment_to_scan_index[static_cast<std::size_t>(scan_index[static_cast<std::size_t>(scan_i)])] = scan_i;
      }
    ...
    for (auto scan_i = best_i + 1; scan_i < static_cast<int>(scan_index.size()); ++scan_i)
      {
        auto const i = scan_index[static_cast<std::size_t>(scan_i)];
        auto const diff = eval.diffs[static_cast<std::size_t>(i)];
        if ((diff == ' ') or (diff == 'A'))
  ...
  for (std::size_t qi = 0; qi < records.size(); ++qi)
    {
      ...
      tav_parallel_for(candidate_indices.size(), tav_aligner_workers, [&](std::size_t const worker_index,
        ...
        c.cigar_left = align_one_end_stock_style(q.left, left_seqno, worker.simd_left, *worker.lma).nwalignment;
        c.cigar_right = align_one_end_stock_style(q.right, right_seqno, worker.simd_right, *worker.lma).nwalignment;
  ...
  if (opt_nonchimeras != nullptr)
    {
      ...
      write_paired_fasta_split(fp_left, fp_right, nonchim, nonchim_scores, "uchime_denovo");
      std::fclose(fp_left);
      std::fclose(fp_right);
    }
  if (opt_chimeras != nullptr)
    {
      ...
      write_paired_fasta_split(fp_left, fp_right, chim, chim_scores, "uchime_denovo");
      std::fclose(fp_left);
      std::fclose(fp_right);
    }
  ...
  if (!opt_quiet)
    {
      std::fprintf(stderr,
                   "TAV uchime3_denovo: %zu input pairs -> %zu nonchimeric, %zu chimeric\n",
                   records.size(),
                   nonchim.size(),
                   chim.size());
    }
...
auto tav_usearch_global(struct Parameters const & parameters, char * cmdline, char * progheader) -> void
{
  if (parameters.opt_interleaved and (parameters.opt_reverse != nullptr))
    {
      fatal("TAV usearch_global query input must be either split (R1 + R2 positional input) or --interleaved, not both");
    }
  if ((not parameters.opt_interleaved) and (parameters.opt_reverse == nullptr))
    {
      fatal("TAV usearch_global requires paired query input: provide R2 as positional input or set --interleaved");
    }
  ...
  if ((fp_notmatched != nullptr) and (fp_notmatched2 == nullptr))
    {
      fatal("Paired usearch_global requires --notmatched2 when --notmatched is provided");
    }
  if ((fp_dbmatched != nullptr) and (fp_dbmatched2 == nullptr))
    {
      fatal("Paired usearch_global requires --dbmatched2 when --dbmatched is provided");
    }
  if ((fp_dbnotmatched != nullptr) and (fp_dbnotmatched2 == nullptr))
    {
      fatal("Paired usearch_global requires --dbnotmatched2 when --dbnotmatched is provided");
    }
  ...
  auto * qr = parameters.opt_interleaved ? nullptr : fastx_open(parameters.opt_reverse);
  if (qf == nullptr)
    {
  ...
  if ((not parameters.opt_interleaved) and (qr == nullptr))
    {
      fatal("Unrecognized split R2 query file type (not proper FASTA or FASTQ format): %s", parameters.opt_reverse);
    }
  ...
  auto const parallel_table_only_mode =
    (parameters.opt_threads > 1) and
    any_otu_table_output and
    (fp_alnout == nullptr) and
    (fp_userout == nullptr) and
    (fp_blast6 == nullptr) and
    (fp_uc == nullptr) and
    (fp_matched == nullptr) and
    (fp_matched2 == nullptr) and
    (fp_notmatched == nullptr) and
    (fp_notmatched2 == nullptr) and
    (fp_dbmatched == nullptr) and
    (fp_dbmatched2 == nullptr) and
    (fp_dbnotmatched == nullptr) and
    (fp_dbnotmatched2 == nullptr);
  ...
  if (parallel_table_only_mode)
    {
      ...
      std::vector<uhandle_s *> query_unique_handles(tav_aligner_workers.size(), nullptr);
      ...
      while (true)
        {
          ...
          while ((batch.size() < query_batch_size) and
                 fastx_next(qf, not opt_notrunclabels, chrmap_no_change_vector.data()))
            {
              ...
              auto const qf_seq = std::string{fastx_get_sequence(qf), static_cast<std::size_t>(qf_len)};

              std::string qr_seq;
              int64_t qr_len = 0;
              if (parameters.opt_interleaved)
                {
                  if (!fastx_next(qf, not opt_notrunclabels, chrmap_no_change_vector.data()))
                    {
                      fatal("Odd number of records in interleaved paired FASTX input %s; expected left/right entries", parameters.opt_usearch_global);
                    }
          ...
          tav_parallel_for(batch.size(), tav_aligner_workers, [&](std::size_t const worker_index,
            ...
            auto const query_search = search_onequery_paired(qrec.left,
                                                             qrec.right,
                                                             db_kmer_index,
                                                             db.size(),
                                                             query_unique_handles[worker_index],
                                                             wordlength,
            ...
            auto process_pending = [&]() -> void {
              ...
              if (!alignable_pending_indices.empty())
                {
                  auto & worker = tav_aligner_workers[worker_index];
                  auto left_stats = align_end_batch_stock_style(qrec.left,
                                                                left_target_seqnos,
                                                                worker.simd_left,
                                                                *worker.lma,
                                                                keep_alignment_strings);
                  auto right_stats = align_end_batch_stock_style(qrec.right,
                                                                 right_target_seqnos,
                                                                 worker.simd_right,
                                                                 *worker.lma,
                                                                 keep_alignment_strings);
                  ...
                  for (std::size_t i = 0; i < alignable_pending_indices.size(); ++i)
                    {
                      ...
                      aligned_pass[pending_index] =
                        paired_aligned_filters_pass(qrec, candidate, hit)
                        ? 1U
                        : 0U;
                    }
                }
            ...
            for (auto const & c : candidates)
              {
                ...
                if (not paired_unaligned_filters_pass(qrec, db[static_cast<std::size_t>(c.first)]))
                  {
                    hits.back().rejected = true;
                    hits.back().weak = false;
                  }
            ...
            if (!report_hits.empty())
              {
                results[batch_index].best_index = report_hits.front()->centroid_index;
              }
          });
              ...
              if (best_index >= 0)
                {
                  ...
                  otutable_add(batch[i].header.c_str(), db[static_cast<std::size_t>(best_index)].header.c_str(), batch[i].abundance);
                }
              else if (opt_unknown_name != nullptr)
                {
                  otutable_add(batch[i].header.c_str(), opt_unknown_name, batch[i].abundance);
                }
              else
                {
  ...
  if ((qr != nullptr) and fastx_next(qr, not opt_notrunclabels, chrmap_no_change_vector.data()))
    {
