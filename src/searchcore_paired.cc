...
auto search_topscores_paired(searchinfo_s_paired *searchinfo, int wordlength)
  ...
  std::memset(searchinfo->kmers, 0,
              static_cast<std::size_t>(indexed_count) * sizeof(unsigned short));
  ...
  auto const kmersamplecount_total =
      searchinfo->kmersamplecount_r1 + searchinfo->kmersamplecount_r2;
  auto const minmatches = std::min(
      static_cast<unsigned int>(opt_minwordmatches), kmersamplecount_total);
  ...
  for (auto i = 0; i < indexed_count; ++i) {
    ...
    if (count >= minmatches) {
      ...
      novel.length = 0U;
      if ((searchinfo->target_lengths != nullptr) and
          (static_cast<std::size_t>(i) < searchinfo->target_lengths->size())) {
        novel.length =
            (*searchinfo->target_lengths)[static_cast<std::size_t>(i)];
      }
...
auto search_onequery_paired(searchinfo_s_paired *searchinfo, int seqmask)
  ...
  unique_count(searchinfo->uh, static_cast<unsigned int>(opt_wordlength),
